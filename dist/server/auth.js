 ...
 */
export function getFeaturePackConfig(request) {
    let token = request.cookies.get('hit_token')?.value;
    if (!token) {
        const authHeader = request.headers.get('authorization');
        if (authHeader?.startsWith('Bearer ')) {
 ...
 */
export function hasRole(user, role, config) {
    if (!user)
        return false;
 ...
 */
export function canChangeStatus(user, config) {
    if (!user)
        return false;
    const hasExtendedRoles = config.support_additional_roles === true;
    if (hasExtendedRoles) {
 ...
 */
export function canManageInventory(user, config) {
    if (!user)
        return false;
    const hasExtendedRoles = config.support_additional_roles === true;
    if (hasExtendedRoles) {
 ...
 */
export function canCompleteRebuild(user, config) {
    if (!user)
        return false;
    const hasExtendedRoles = config.support_additional_roles === true;
    if (hasExtendedRoles) {
 ...
 */
export function canEnterFieldData(user, config) {
    if (!user)
        return false;
    const hasExtendedRoles = config.support_additional_roles === true;
    if (hasExtendedRoles) {
 ...
 */
export function canViewAnalytics(user, config) {
    if (!user)
        return false;
    const hasExtendedRoles = config.support_additional_roles === true;
    if (hasExtendedRoles) {
