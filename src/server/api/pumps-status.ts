...
export async function POST(request: NextRequest) {
  try {
    ...
    const currentStatus = pump.status as PumpStatus;
    ...
    const transitionValidation = validateStatusTransition(currentStatus, newStatus as PumpStatus);
    ...
    const roleValidation = validateRoleForStatusChange(userRole, currentStatus, newStatus as PumpStatus, config);
    ...
    await db.insert(equipmentActivityLog).values({
      ...
      oldStatus: currentStatus,
      newStatus: newStatus as PumpStatus,
      notes: notes || '',
      timestamp: new Date(),
    });
